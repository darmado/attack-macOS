...
USE_THE_TEXTUTIL_TO_READ_SEVERAL_FILES_AND_BUILD_A_NEW_FILE=false
CAPTURE_CLIPBOARD_CONTENT=false

CMD_TEXTUTIL="/usr/bin/textutil"
...
PROCEDURE_NAME="textutil"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --use-the-textutil-to-read-several-files-and-build-a-new-file)
            USE_THE_TEXTUTIL_TO_READ_SEVERAL_FILES_AND_BUILD_A_NEW_FILE=true
            ;;
...
SCRIPT:
  --use-the-textutil-to-read-several-files-and-build-a-new-file A one-liner can load the content of multiple RTF files in a directory, concatenate their contents, a
  --capture-clipboard-content      By leveraging another command line tool, pbpaste, it is possible to write a one-liner which captures
...
# Execute functions for --use-the-textutil-to-read-several-files-and-build-a-new-file
if [ "$USE_THE_TEXTUTIL_TO_READ_SEVERAL_FILES_AND_BUILD_A_NEW_FILE" = true ]; then
    core_debug_print "Executing functions for --use-the-textutil-to-read-several-files-and-build-a-new-file"
    result=$(execute_function execute_use_textutil_read_several_files_build_new_file)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --capture-clipboard-content
if [ "$CAPTURE_CLIPBOARD_CONTENT" = true ]; then
    core_debug_print "Executing functions for --capture-clipboard-content"
    result=$(execute_function execute_capture_clipboard_content)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_use_textutil_read_several_files_build_new_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_textutil_read_several_files_build_new_file() {
    local result
    result=$(textutil -convert html Quote.doc secondQuote.doc 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_capture_clipboard_content
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_capture_clipboard_content() {
    local result
    result=$(pbpaste | textutil -stdin -info > Clipboard.txt 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
