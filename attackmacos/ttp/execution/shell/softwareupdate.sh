...
TTP_ID="T1072" #replace with you coresponding ttp_id
...
GET_OS_AND_BROWSER_VERSION_INFORMATION=false
GET_OS_UPDATE_POLICY=false

CMD_SOFTWAREUPDATE="/usr/sbin/softwareupdate"
...
PROCEDURE_NAME="softwareupdate"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --get-os-and-browser-version-information)
            GET_OS_AND_BROWSER_VERSION_INFORMATION=true
            ;;
        --get-os-update-policy)
            GET_OS_UPDATE_POLICY=true
            ;;
            *)
...
SCRIPT:
  --get-os-and-browser-version-information Determine OS and Safari version by enumerating the available software updates.
  --get-os-update-policy           Use the --schedule flag to return the OS update policy.
...
# Execute functions for --get-os-and-browser-version-information
if [ "$GET_OS_AND_BROWSER_VERSION_INFORMATION" = true ]; then
    core_debug_print "Executing functions for --get-os-and-browser-version-information"
    result=$(execute_function execute_get_os_browser_version_information)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --get-os-update-policy
if [ "$GET_OS_UPDATE_POLICY" = true ]; then
    core_debug_print "Executing functions for --get-os-update-policy"
    result=$(execute_function execute_get_os_update_policy)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="softwareupdate"
...
# Function: execute_get_os_browser_version_information
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_get_os_browser_version_information() {
    local result
    result=$(softwareupdate --list 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_get_os_update_policy
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_get_os_update_policy() {
    local result
    result=$(softwareupdate --schedule 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
