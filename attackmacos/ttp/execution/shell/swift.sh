...
TTP_ID="T1059" #replace with you coresponding ttp_id
...
EXECUTE_SWIFT_CODE_FILE=false
EXECUTE_SWIFT_ONE_LINER_STDIN=false
EXECUTE_SWIFT_ONE_LINER_E_FLAG=false

CMD_SWIFT="/usr/bin/swift"
...
PROCEDURE_NAME="swift"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --execute-swift-code-file)
            EXECUTE_SWIFT_CODE_FILE=true
            ;;
        --execute-swift-one-liner-stdin)
            EXECUTE_SWIFT_ONE_LINER_STDIN=true
            ;;
        --execute-swift-one-liner-e-flag)
            EXECUTE_SWIFT_ONE_LINER_E_FLAG=true
            ;;
            *)
...
SCRIPT:
  --execute-swift-code-file        Executes the Swift code that is in a .swift file
  --execute-swift-one-liner-stdin  Swift one-liner via stdin (echo … | swift -). Older Swift/Xcode before swift -e became common.
  --execute-swift-one-liner-e-flag Swift one-liner using swift -e (Swift 5.8+ / modern Xcode).
...
# Execute functions for --execute-swift-code-file
if [ "$EXECUTE_SWIFT_CODE_FILE" = true ]; then
    core_debug_print "Executing functions for --execute-swift-code-file"
    result=$(execute_function execute_execute_swift_code_file)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --execute-swift-one-liner-stdin
if [ "$EXECUTE_SWIFT_ONE_LINER_STDIN" = true ]; then
    core_debug_print "Executing functions for --execute-swift-one-liner-stdin"
    result=$(execute_function execute_execute_swift_one_liner_before_swift_5_8_xcode_14_3_beta_1)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --execute-swift-one-liner-e-flag
if [ "$EXECUTE_SWIFT_ONE_LINER_E_FLAG" = true ]; then
    core_debug_print "Executing functions for --execute-swift-one-liner-e-flag"
    result=$(execute_function execute_execute_swift_one_liner_swift_5_8_xcode_14_3_beta_1_greater)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_execute_swift_code_file
# Type: main
# Languages: swift
FUNCTION_LANG="swift"
# Sudo privileges: Not required

execute_execute_swift_code_file() {
    local result
    result=$(swift mycode.swift 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_execute_swift_one_liner_before_swift_5_8_xcode_14_3_beta_1
# Type: main
# Languages: swift
FUNCTION_LANG="swift"
# Sudo privileges: Not required

execute_execute_swift_one_liner_before_swift_5_8_xcode_14_3_beta_1() {
    local result
    result=$(echo 'print("loobins")' | swift - 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_execute_swift_one_liner_swift_5_8_xcode_14_3_beta_1_greater
# Type: main
# Languages: swift
FUNCTION_LANG="swift"
# Sudo privileges: Not required

execute_execute_swift_one_liner_swift_5_8_xcode_14_3_beta_1_greater() {
    local result
    result=$(swift -e 'import Foundation; let process = Process(); process.executableURL = URL(fileURLWithPath:"/bin/bash"); process.arguments = ["-c", "ls -alh"]; let stdout = Pipe(); let stderr = Pipe(); process.standardOutput = stdout; process.standardError = stderr; try process.run(); print(String(decoding: stdout.fileHandleForReading.readDataToEndOfFile(), as: UTF8.self)); print(String(decoding: stderr.fileHandleForReading.readDataToEndOfFile(), as: UTF8.self));' 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
