#!/bin/sh
# POSIX-compliant
# Procedure Name: safaridriver
# Tactic: Execution
# Technique: T1185
# GUID: 914e9f5f-ad21-4ab9-a7d3-97a2839951ea
# Intent: Enable the WebDriver Safari browser API for Selenium testing. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
TTP_ID="T1185" #replace with you coresponding ttp_id
...
ENABLE_SAFARIDRIVER=false

CMD_SAFARIDRIVER="/System/Cryptexes/App/usr/bin/safaridriver"
...
PROCEDURE_NAME="safaridriver"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --enable-safaridriver            The following command can be used to enable the WebDriver Safari browser API. The command must be ru
...
# Execute functions for --enable-safaridriver
if [ "$ENABLE_SAFARIDRIVER" = true ]; then
    core_debug_print "Executing functions for --enable-safaridriver"
    result=$(execute_function execute_enable_safaridriver)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="safaridriver"
...
# Function: execute_enable_safaridriver
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_enable_safaridriver() {
    local result
    result=$(sudo safaridriver --enable 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
