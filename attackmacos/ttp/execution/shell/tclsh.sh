...
PROCEDURE_NAME="tclsh"  # Set by build system from YAML procedure_name field
...
execute_execute_malicious_dynamic_library_dylib_from_standard_input() {
    local result
    result=$(echo "load bad.dylib" | tclsh 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
