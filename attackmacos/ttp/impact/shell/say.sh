...
TACTIC="Impact" #replace with you coresponding tactic
TTP_ID="T1491" #replace with you coresponding ttp_id
...
PROCEDURE_NAME="say"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --read-sensitive-data            The following command can read and process sensitive files and redirects the output to a file..
  --collect-clipboard-data         The command is designed to enhance privacy by muting the system volume,using a less recognizable "Wh
...
# Execute functions for --read-sensitive-data
if [ "$READ_SENSITIVE_DATA" = true ]; then
    core_debug_print "Executing functions for --read-sensitive-data"
    result=$(execute_function execute_read_sensitive_data)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --collect-clipboard-data
if [ "$COLLECT_CLIPBOARD_DATA" = true ]; then
    core_debug_print "Executing functions for --collect-clipboard-data"
    result=$(execute_function execute_collect_clipboard_data)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_read_sensitive_data
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_read_sensitive_data() {
    local result
    result=$(say -f /home/user/sensitive-files -i  > loot.txt; 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_collect_clipboard_data
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_collect_clipboard_data() {
    local result
    result=$(osascript -e 'set volume output muted true' ;   say $(pbpaste) -i  > loot.txt; 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
