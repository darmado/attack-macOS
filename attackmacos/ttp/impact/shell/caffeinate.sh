...
TTP_ID="T1499" #replace with you coresponding ttp_id
...
FORK_A_PROCESS=false
PREVENT_A_SLEEP=false

CMD_CAFFEINATE="/usr/bin/caffeinate"
...
PROCEDURE_NAME="caffeinate"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --fork-a-process                 Make caffeinate fork a process and hold an assertion that prevents idle sleep as long as that proces
  --prevent-a-sleep                Prevent a macOS from going to sleep for 4 hours (14400 seconds)
...
# Execute functions for --fork-a-process
if [ "$FORK_A_PROCESS" = true ]; then
    core_debug_print "Executing functions for --fork-a-process"
    result=$(execute_function execute_fork_process)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --prevent-a-sleep
if [ "$PREVENT_A_SLEEP" = true ]; then
    core_debug_print "Executing functions for --prevent-a-sleep"
    result=$(execute_function execute_prevent_sleep)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="caffeinate"
...
# Function: execute_fork_process
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_fork_process() {
    local result
    result=$(caffeinate -i /tmp/evil 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_prevent_sleep
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_prevent_sleep() {
    local result
    result=$(caffeinate -u -t 14400 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
