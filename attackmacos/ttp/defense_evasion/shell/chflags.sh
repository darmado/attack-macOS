...
HIDE_A_FILE=false
REMOVE_HIDDEN_FLAG=false

CMD_CHFLAGS="/usr/bin/chflags"
...
PROCEDURE_NAME="chflags"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --hide-a-file                    Add the hidden flag to a file or directory to prevent it from being 
visible in Finder and Terminal.
  --remove-hidden-flag             Remove the hidden flag to a file or directory to make it visible in Finder
and Terminal.
...
# Execute functions for --hide-a-file
if [ "$HIDE_A_FILE" = true ]; then
    core_debug_print "Executing functions for --hide-a-file"
    result=$(execute_function execute_hide_file)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --remove-hidden-flag
if [ "$REMOVE_HIDDEN_FLAG" = true ]; then
    core_debug_print "Executing functions for --remove-hidden-flag"
    result=$(execute_function execute_remove_hidden_flag)
    raw_output="${raw_output}${result}"
fi
...
execute_hide_file() {
    local result
    result=$(chflags hidden ~/evil 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_remove_hidden_flag
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_remove_hidden_flag() {
    local result
    result=$(chflags nohidden ~/evil 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
