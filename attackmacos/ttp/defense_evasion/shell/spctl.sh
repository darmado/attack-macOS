...
PROCEDURE_NAME="spctl"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --disable-gatekeeper             The --master-disable switch disables Gatekeeper. The command must be run with root/sudo permission.
...
if [ "$DISABLE_GATEKEEPER" = true ]; then
    ...
    result=$(execute_function execute_disable_gatekeeper)
...
# Function: execute_disable_gatekeeper
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_disable_gatekeeper() {
    local result
    result=$(sudo spctl --master-disable 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
