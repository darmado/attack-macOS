...
TTP_ID="T1564.004" #replace with you coresponding ttp_id
...
MOUNT_A_MALICIOUS_DMG_FILE=false
MOUNT_A_MALICIOUS_DMG_FILE=false
MOUNT_A_MALICIOUS_ISO_FILE=false
MOUNT_A_MALICIOUS_ISO_FILE=false
EXFILTRATE_DATA_IN_DMG_FILE=false
EXFILTRATE_DATA_IN_ENCRYPTED_DMG_FILE=false
...
PROCEDURE_NAME="hdiutil"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --mount-a-malicious-dmg-file)
            MOUNT_A_MALICIOUS_DMG_FILE=true
            ;;
        --mount-a-malicious-dmg-file)
            MOUNT_A_MALICIOUS_DMG_FILE=true
            ;;
        --mount-a-malicious-iso-file)
            MOUNT_A_MALICIOUS_ISO_FILE=true
            ;;
        --mount-a-malicious-iso-file)
            MOUNT_A_MALICIOUS_ISO_FILE=true
            ;;
        --exfiltrate-data-in-dmg-file)
            EXFILTRATE_DATA_IN_DMG_FILE=true
            ;;
        --exfiltrate-data-in-encrypted-dmg-file)
            EXFILTRATE_DATA_IN_ENCRYPTED_DMG_FILE=true
            ;;
            *)
...
SCRIPT:
  --mount-a-malicious-dmg-file     Uses hdiutil to mount a malicious dmg file to
  --mount-a-malicious-dmg-file     Uses hdiutil to mount a malicious dmg file to
  --mount-a-malicious-iso-file     Uses hdiutil to mount a malicious iso file to
  --mount-a-malicious-iso-file     Uses hdiutil to mount a malicious iso file to
  --exfiltrate-data-in-dmg-file    Uses hdiutil to create a dmg file to store exfiltrate data
  --exfiltrate-data-in-encrypted-dmg-file Uses hdiutil to create a dmg file to store exfiltrate data
...
# Execute functions for --mount-a-malicious-dmg-file
if [ "$MOUNT_A_MALICIOUS_DMG_FILE" = true ]; then
    core_debug_print "Executing functions for --mount-a-malicious-dmg-file"
    result=$(execute_function execute_mount_malicious_dmg_file)
...
# Execute functions for --mount-a-malicious-iso-file
if [ "$MOUNT_A_MALICIOUS_ISO_FILE" = true ]; then
    core_debug_print "Executing functions for --mount-a-malicious-iso-file"
    result=$(execute_function execute_mount_malicious_iso_file)
    raw_output="${raw_output}${result}"
fi
...
# Execute functions for --exfiltrate-data-in-dmg-file
if [ "$EXFILTRATE_DATA_IN_DMG_FILE" = true ]; then
    core_debug_print "Executing functions for --exfiltrate-data-in-dmg-file"
    result=$(execute_function execute_exfiltrate_data_dmg_file)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --exfiltrate-data-in-encrypted-dmg-file
if [ "$EXFILTRATE_DATA_IN_ENCRYPTED_DMG_FILE" = true ]; then
    core_debug_print "Executing functions for --exfiltrate-data-in-encrypted-dmg-file"
    result=$(execute_function execute_exfiltrate_data_encrypted_dmg_file)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_mount_malicious_dmg_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_mount_malicious_dmg_file() {
    local result
    result=$(hdiutil mount malicious.dmg 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
...
execute_mount_malicious_dmg_file() {
    local result
    result=$(hdiutil attach malicious.dmg 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_mount_malicious_iso_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_mount_malicious_iso_file() {
    local result
    result=$(hdiutil mount malicious.iso 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
...
execute_mount_malicious_iso_file() {
    local result
    result=$(hdiutil attach malicious.iso 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_exfiltrate_data_dmg_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_exfiltrate_data_dmg_file() {
    local result
    result=$(hdiutil create -volname "Volume Name" -srcfolder /path/to/folder -ov diskimage.dmg 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_exfiltrate_data_encrypted_dmg_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_exfiltrate_data_encrypted_dmg_file() {
    local result
    result=$(hdiutil create -encryption -stdinpass -volname "Volume Name" -srcfolder /path/to/folder -ov encrypteddiskimage.dmg 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
