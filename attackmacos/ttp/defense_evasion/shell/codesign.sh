...
TTP_ID="T1553" #replace with you coresponding ttp_id
...
AD_HOD_CODESIGNING_AN_APP_BUNDLE=false

CMD_CODESIGN="/usr/bin/codesign"
...
PROCEDURE_NAME="codesign"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --ad-hod-codesigning-an-app-bundle)
            AD_HOD_CODESIGNING_AN_APP_BUNDLE=true
            ;;
            *)
...
SCRIPT:
  --ad-hod-codesigning-an-app-bundle This command forcefully re-signs the MyApp.app application with an ad-hoc signature, applying the si
...
# Execute functions for --ad-hod-codesigning-an-app-bundle
if [ "$AD_HOD_CODESIGNING_AN_APP_BUNDLE" = true ]; then
    core_debug_print "Executing functions for --ad-hod-codesigning-an-app-bundle"
    result=$(execute_function execute_ad_hod_codesigning_app_bundle)
...
# Function: execute_ad_hod_codesigning_app_bundle
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_ad_hod_codesigning_app_bundle() {
    local result
    result=$(codesign --force --deep -s - MyApp.app 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
