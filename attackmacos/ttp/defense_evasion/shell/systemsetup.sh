...
ENABLE_REMOTE_APPLE_EVENTS=false

CMD_SYSTEMSETUP="/usr/sbin/systemsetup"
...
PROCEDURE_NAME="systemsetup"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        ...
        --enable-remote-apple-events)
            ENABLE_REMOTE_APPLE_EVENTS=true
            ;;
            *)
...
SCRIPT:
  --enable-remote-login            systemsetup can be used to enable SSH for remote login
  --enable-remote-apple-events     systemsetup can be used to enable Remote Apple Events. 
Set whether the system responds to events se
...
# Execute functions for --enable-remote-login
if [ "$ENABLE_REMOTE_LOGIN" = true ]; then
    core_debug_print "Executing functions for --enable-remote-login"
    result=$(execute_function execute_enable_remote_login)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --enable-remote-apple-events
if [ "$ENABLE_REMOTE_APPLE_EVENTS" = true ]; then
    core_debug_print "Executing functions for --enable-remote-apple-events"
    result=$(execute_function execute_enable_remote_apple_events)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_enable_remote_login
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_enable_remote_login() {
    local result
    result=$(sudo systemsetup -setremotelogin on
 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_enable_remote_apple_events
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_enable_remote_apple_events() {
    local result
    result=$(sudo systemsetup -setremoteappleevents on
 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
