...
TTP_ID="T1553.004" #replace with you coresponding ttp_id
...
BYPASS_GATEKEEPER_VIA_XATTR=false
BYPASS_GATEKEEPER_VIA_XATTR=false

CMD_XATTR="/usr/bin/xattr"
...
PROCEDURE_NAME="xattr"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --bypass-gatekeeper-via-xattr)
            BYPASS_GATEKEEPER_VIA_XATTR=true
            ;;
        --bypass-gatekeeper-via-xattr)
            BYPASS_GATEKEEPER_VIA_XATTR=true
            ;;
...
SCRIPT:
  --bypass-gatekeeper-via-xattr    Use xattr to remove quaratine extended attribute from a file.
  --bypass-gatekeeper-via-xattr    Use xattr to remove quaratine extended attribute from multiple files or directories.
...
# Execute functions for --bypass-gatekeeper-via-xattr
if [ "$BYPASS_GATEKEEPER_VIA_XATTR" = true ]; then
    core_debug_print "Executing functions for --bypass-gatekeeper-via-xattr"
    result=$(execute_function execute_bypass_gatekeeper_via_xattr)
...
# Function: execute_bypass_gatekeeper_via_xattr
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_bypass_gatekeeper_via_xattr() {
    local result
    result=$(xattr -d com.apple.quarantine FILE 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
...
execute_bypass_gatekeeper_via_xattr() {
    local result
    result=$(xattr -d -r com.apple.quarantine * 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
