...
TTP_ID="T1562.001" #replace with you coresponding ttp_id
...
DISABLE_GATEKEEPER_S_AUTO_REARM_FUNCTIONALITY=false
SHOW_MOUNTED_SERVERS=false
ADD_A_LOGIN_ITEM_TO_THE_CURRENT_USER=false
GET_ACTIVE_DIRECTORY_USER_INFO_FROM_JAMF_CONNECT=false
...
PROCEDURE_NAME="defaults"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --disable-gatekeeper-s-auto-rearm-functionality)
            DISABLE_GATEKEEPER_S_AUTO_REARM_FUNCTIONALITY=true
            ;;
        --show-mounted-servers)
            SHOW_MOUNTED_SERVERS=true
            ;;
        --add-a-login-item-to-the-current-user)
            ADD_A_LOGIN_ITEM_TO_THE_CURRENT_USER=true
            ;;
        --get-active-directory-user-info-from-jamf-connect)
            GET_ACTIVE_DIRECTORY_USER_INFO_FROM_JAMF_CONNECT=true
            ;;
        --enable-firewall)
...
SCRIPT:
  --disable-gatekeeper-s-auto-rearm-functionality The following command can be used to disable Gatekeepers rearm functionality. This command requires 
  --show-mounted-servers           Show all mounted servers on the desktop.
  --add-a-login-item-to-the-current-user An attacker can use defaults to add a login hook in attempt to gain persistence. This command requir
  --get-active-directory-user-info-from-jamf-connect Retrieve Active Directory user info from Jamf Connect defaults configuration.
  --enable-firewall                Enables macOS' default firewall. This command requires root privileges.
  --disable-firewall               Disables macOS' default firewall. This command requires root privileges.
...
# Execute functions for --disable-gatekeeper-s-auto-rearm-functionality
if [ "$DISABLE_GATEKEEPER_S_AUTO_REARM_FUNCTIONALITY" = true ]; then
    core_debug_print "Executing functions for --disable-gatekeeper-s-auto-rearm-functionality"
    result=$(execute_function execute_disable_gatekeeper_s_auto_rearm_functionality)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --show-mounted-servers
if [ "$SHOW_MOUNTED_SERVERS" = true ]; then
    core_debug_print "Executing functions for --show-mounted-servers"
    result=$(execute_function execute_show_mounted_servers)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --add-a-login-item-to-the-current-user
if [ "$ADD_A_LOGIN_ITEM_TO_THE_CURRENT_USER" = true ]; then
    core_debug_print "Executing functions for --add-a-login-item-to-the-current-user"
    result=$(execute_function execute_add_login_item_current_user)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --get-active-directory-user-info-from-jamf-connect
if [ "$GET_ACTIVE_DIRECTORY_USER_INFO_FROM_JAMF_CONNECT" = true ]; then
    core_debug_print "Executing functions for --get-active-directory-user-info-from-jamf-connect"
    result=$(execute_function execute_get_active_directory_user_info_from_jamf_connect)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --enable-firewall
if [ "$ENABLE_FIREWALL" = true ]; then
    core_debug_print "Executing functions for --enable-firewall"
    result=$(execute_function execute_enable_firewall)
    raw_output="${raw_output}${result}"
fi
...
if [ "$DISABLE_FIREWALL" = true ]; then
    ...
    result=$(execute_function execute_disable_firewall)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_disable_gatekeeper_s_auto_rearm_functionality
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_disable_gatekeeper_s_auto_rearm_functionality() {
    local result
    result=$(sudo defaults write /Library/Preferences/com.apple.security GKAutoRearm -bool NO 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_show_mounted_servers
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_show_mounted_servers() {
    local result
    result=$(defaults read com.apple.finder "ShowMountedServersOnDesktop" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_add_login_item_current_user
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_add_login_item_current_user() {
    local result
    result=$(sudo defaults write /Library/Preferences/com.apple.loginwindow LoginHook gain_persistence.sh 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_get_active_directory_user_info_from_jamf_connect
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_get_active_directory_user_info_from_jamf_connect() {
    local result
    result=$(defaults read com.jamf.connect.state 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_enable_firewall
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_enable_firewall() {
    local result
    result=$(sudo defaults write /Library/Preferences/com.apple.alf globalstate -int 1 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_disable_firewall
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_disable_firewall() {
    local result
    result=$(sudo defaults write /Library/Preferences/com.apple.alf globalstate -int 0 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
