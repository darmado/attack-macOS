...
COLLECT_SYSTEM_DEP_INFORMATION=false
REMOVE_CONFIGURATION_PROFILES=false
...
PROCEDURE_NAME="profiles"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --collect-system-dep-information)
            COLLECT_SYSTEM_DEP_INFORMATION=true
            ;;
        --remove-configuration-profiles)
            REMOVE_CONFIGURATION_PROFILES=true
            ;;
            *)
...
SCRIPT:
  --collect-system-dep-information The following command determines whether device is DEP(Device Enrolment Program) enabled and output 
  --remove-configuration-profiles  The following command deletes the specified profiles. An optional password used when removing a conf
...
# Execute functions for --collect-system-dep-information
if [ "$COLLECT_SYSTEM_DEP_INFORMATION" = true ]; then
    core_debug_print "Executing functions for --collect-system-dep-information"
    result=$(execute_function execute_collect_system_dep_information)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --remove-configuration-profiles
if [ "$REMOVE_CONFIGURATION_PROFILES" = true ]; then
    core_debug_print "Executing functions for --remove-configuration-profiles"
    result=$(execute_function execute_remove_configuration_profiles)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_collect_system_dep_information
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_collect_system_dep_information() {
    local result
    result=$(sudo profiles show -type enrollment 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_remove_configuration_profiles
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_remove_configuration_profiles() {
    local result
    result=$(profiles remove -identifier com.profile.identifier -password <password> 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
