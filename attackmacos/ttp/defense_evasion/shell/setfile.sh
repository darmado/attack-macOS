...
TTP_ID="T1564.001" #replace with you coresponding ttp_id
...
SET_A_FILE_OR_DIRECTORY_ATTRIBUTE_TO_INVISIBLE=false
CHANGE_A_FILE_S_CREATION_AND_MODIFICATION_TIMESTAMPS=false
...
PROCEDURE_NAME="setfile"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --set-a-file-or-directory-attribute-to-invisible)
            SET_A_FILE_OR_DIRECTORY_ATTRIBUTE_TO_INVISIBLE=true
            ;;
        --change-a-file-s-creation-and-modification-timestamps)
            CHANGE_A_FILE_S_CREATION_AND_MODIFICATION_TIMESTAMPS=true
            ;;
            *)
...
SCRIPT:
  --set-a-file-or-directory-attribute-to-invisible A bash or zsh oneliner can allow an attacker to set the file attribute to invisible. This action can
  --change-a-file-s-creation-and-modification-timestamps Setfile can be used with the -d and -m arguments to alter a file's creation and modification date, r
...
# Execute functions for --set-a-file-or-directory-attribute-to-invisible
if [ "$SET_A_FILE_OR_DIRECTORY_ATTRIBUTE_TO_INVISIBLE" = true ]; then
    core_debug_print "Executing functions for --set-a-file-or-directory-attribute-to-invisible"
    result=$(execute_function execute_set_file_directory_attribute_invisible)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --change-a-file-s-creation-and-modification-timestamps
if [ "$CHANGE_A_FILE_S_CREATION_AND_MODIFICATION_TIMESTAMPS" = true ]; then
    core_debug_print "Executing functions for --change-a-file-s-creation-and-modification-timestamps"
    result=$(execute_function execute_change_file_s_creation_modification_timestamps)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_set_file_directory_attribute_invisible
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_set_file_directory_attribute_invisible() {
    local result
    result=$(for FILE in ~/*; do echo $(SetFile -a V $FILE && echo $(GetFileInfo $FILE)) >> /tmp/fileinfo.txt; sleep 2; done 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_change_file_s_creation_modification_timestamps
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_change_file_s_creation_modification_timestamps() {
    local result
    result=$(SetFile -d "04/25/2023 11:11:00" -m "04/25/2023 11:12:00" targetfile.txt 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
