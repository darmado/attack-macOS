...
INPUT_LAST="${INPUT_LAST:-7}"
INPUT_SEARCH="${INPUT_SEARCH:-}"
CMD_SQLITE3="${CMD_SQLITE3:-sqlite3}"
DB_HISTORY_SAFARI="${DB_HISTORY_SAFARI:-$HOME/Library/Safari/History.db}"
DB_HISTORY_CHROME="${DB_HISTORY_CHROME:-$HOME/Library/Application Support/Google/Chrome/Default/History}"
CMD_QUERY_BROWSER_DB="${CMD_QUERY_BROWSER_DB:-$CMD_SQLITE3 -separator '|'}"
DB_HISTORY_BRAVE="${DB_HISTORY_BRAVE:-$HOME/Library/Application Support/BraveSoftware/Brave-Browser/Default/History}"
DB_HISTORY_FIREFOX="${DB_HISTORY_FIREFOX:-$HOME/Library/Application Support/Firefox/Profiles/*.default-release/places.sqlite}"
SAFARI_HDB_QUERY="${SAFARI_HDB_QUERY:-"
    WITH headers AS (
...
CHROME_HDB_QUERY="${CHROME_HDB_QUERY:-"
    WITH headers AS (
...
FIREFOX_HDB_QUERY="${FIREFOX_HDB_QUERY:-"
    WITH headers AS (
        SELECT 'source' as source, 'url' as url, 'title' as title, 
            'visit_date' as visit_date, 'visit_count' as visit_count
    )
...
BRAVE_HDB_QUERY="${BRAVE_HDB_QUERY:-"
    WITH headers AS (
        SELECT 'source' as source, 'url' as url, 'title' as title, 
            'visit_date' as visit_date, 'visit_count' as visit_count
    )
...
#Parameters: None
#Returns: Query results on stdout
#Dependencies: Global command variables, core_debug_print
...
# Validate procedure-specific arguments
# Skipping validation for search parameter: --search

if [ -n "$INPUT_LAST" ]; then
    if ! core_validate_input "$INPUT_LAST" "integer"; then
        validation_errors="${validation_errors}Invalid --last: $INPUT_LAST\n"
    fi
fi

if [ -n "$INPUT_STARTTIME" ]; then
    if ! core_validate_input "$INPUT_STARTTIME" "string"; then
        validation_errors="${validation_errors}Invalid --starttime: $INPUT_STARTTIME\n"
    fi
fi

if [ -n "$INPUT_ENDTIME" ]; then
    if ! core_validate_input "$INPUT_ENDTIME" "string"; then
        validation_errors="${validation_errors}Invalid --endtime: $INPUT_ENDTIME\n"
    fi
fi
...
if [ "$SAFARI" = true ]; then
    ...
    local query_safari_history_output
    query_safari_history_output=$(query_safari_history) || {
        core_debug_print "Function query_safari_history failed"
        continue
    }
    if [ -n "${query_safari_history_output}" ]; then
        raw_output="${raw_output}${query_safari_history_output}"
...
if [ "$CHROME" = true ]; then
    ...
    local query_chrome_history_output
    query_chrome_history_output=$(query_chrome_history) || {
        core_debug_print "Function query_chrome_history failed"
        continue
    }
    if [ -n "${query_chrome_history_output}" ]; then
        raw_output="${raw_output}${query_chrome_history_output}"
        raw_output="${raw_output}\n"
    fi
fi
...
if [ "$FIREFOX" = true ]; then
    ...
    local query_firefox_history_output
    query_firefox_history_output=$(query_firefox_history) || {
        core_debug_print "Function query_firefox_history failed"
        continue
    }
    if [ -n "${query_firefox_history_output}" ]; then
        raw_output="${raw_output}${query_firefox_history_output}"
        raw_output="${raw_output}\n"
    fi
fi
...
if [ "$BRAVE" = true ]; then
    ...
    local query_brave_history_output
    query_brave_history_output=$(query_brave_history) || {
        core_debug_print "Function query_brave_history failed"
        continue
    }
    if [ -n "${query_brave_history_output}" ]; then
        raw_output="${raw_output}${query_brave_history_output}"
        raw_output="${raw_output}\n"
    fi
fi
