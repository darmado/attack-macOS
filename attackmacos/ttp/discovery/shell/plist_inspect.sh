#!/bin/sh
# POSIX-compliant
# Procedure Name: plist_inspect
# Tactic: Discovery
# Technique: T1082
# GUID: 6a8911ba-df91-41c9-9cac-cff71b6a4d3d
# Intent: Read, create or edit plist files. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
SET_APP_TO_RUN_WITH_DOCK_ICON_HIDDEN=false
...
PROCEDURE_NAME="plist_inspect"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --set-app-to-run-with-dock-icon-hidden)
            SET_APP_TO_RUN_WITH_DOCK_ICON_HIDDEN=true
            ;;
            *)
...
SCRIPT:
  --set-app-to-run-with-dock-icon-hidden plutil can be used to set the "LSUIElement" attribute to tue which will force the targeted app to ru
...
# Execute functions for --set-app-to-run-with-dock-icon-hidden
if [ "$SET_APP_TO_RUN_WITH_DOCK_ICON_HIDDEN" = true ]; then
    core_debug_print "Executing functions for --set-app-to-run-with-dock-icon-hidden"
    result=$(execute_function execute_set_app_run_dock_icon_hidden)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="plist_inspect"
...
# Function: execute_set_app_run_dock_icon_hidden
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_set_app_run_dock_icon_hidden() {
    local result
    result=$(plutil -insert LSUIElement -string "1" /Applications/TargetApp.app/Contents/Info.plist 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
