#!/bin/sh
# POSIX-compliant
# Procedure Name: emails
# Tactic: Discovery
# Technique: T1087.003
# GUID: 1650abc4-b8f5-489a-8c3e-f5e3cf6499cb
...
TTP_ID="T1087.003" #replace with you coresponding ttp_id
...
INPUT_DOMAIN=""
DB_MESSAGES="$HOME/Library/Messages/chat.db"
DB_ENTITIES="$HOME/Library/Suggestions/entities.db"
DB_WHATSAPP_CHAT="$HOME/Library/Group Containers/group.net.whatsapp.WhatsApp.shared/ChatStorage.sqlite"

# Procedure Information (set by build system)
PROCEDURE_NAME="emails"  # Set by build system from YAML procedure_name field
...
Basic Options:
  ...
  --messages                Extract emails from Apple Messages database
  --entities                Extract emails from Apple Suggestions database
  --whatsapp                Extract emails from WhatsApp message content
  --all                     Extract emails from all verified sources
...
# Execute functions for --messages
if [ "$MESSAGES" = true ]; then
    core_debug_print "Executing functions for --messages"
    result=$(discover_messages_emails)
    raw_output="${raw_output}${result}\n"
fi

# Execute functions for --entities
if [ "$ENTITIES" = true ]; then
    core_debug_print "Executing functions for --entities"
    result=$(discover_entities_emails)
    raw_output="${raw_output}${result}\n"
fi

# Execute functions for --whatsapp
if [ "$WHATSAPP" = true ]; then
    core_debug_print "Executing functions for --whatsapp"
    result=$(discover_whatsapp_emails)
    raw_output="${raw_output}${result}\n"
fi
...
# Function: discover_messages_emails
# Description: discover_messages_emails - Generated from YAML
...
# Function: discover_entities_emails
# Description: discover_entities_emails - Generated from YAML
...
# Function: discover_whatsapp_emails
# Description: discover_whatsapp_emails - Generated from YAML
