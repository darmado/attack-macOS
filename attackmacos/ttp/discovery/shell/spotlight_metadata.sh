#!/bin/sh
# POSIX-compliant
# Procedure Name: spotlight_metadata
# Tactic: Discovery
# Technique: T1083
...
VALIDATE_FILE_DOWNLOAD_INFORMATION=false
QUERY_FILE_PATHS=false

CMD_MDLS="/usr/bin/mdls"
...
PROCEDURE_NAME="spotlight_metadata"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --validate-file-download-information)
            VALIDATE_FILE_DOWNLOAD_INFORMATION=true
            ;;
        --query-file-paths)
            QUERY_FILE_PATHS=true
            ;;
            *)
...
SCRIPT:
  --validate-file-download-information Use mdls to validate payload download sources and timestamps to guard against sandbox executions.
  --query-file-paths               Use mdls to print file paths and sizes when enumerating host resources.
...
# Execute functions for --validate-file-download-information
if [ "$VALIDATE_FILE_DOWNLOAD_INFORMATION" = true ]; then
    core_debug_print "Executing functions for --validate-file-download-information"
    result=$(execute_function execute_validate_file_download_information)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --query-file-paths
if [ "$QUERY_FILE_PATHS" = true ]; then
    core_debug_print "Executing functions for --query-file-paths"
    result=$(execute_function execute_query_file_paths)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="spotlight_metadata"
...
# Function: execute_validate_file_download_information
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_validate_file_download_information() {
    local result
    result=$(mdls -name "kMDItemWhereFroms" -name "kMDItemDownloadedDate" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_query_file_paths
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_query_file_paths() {
    local result
    result=$(xargs -0 mdls -n kMDItemPath -n kMDItemFSSize 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
