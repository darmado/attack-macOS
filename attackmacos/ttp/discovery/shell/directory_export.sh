#!/bin/sh
# POSIX-compliant
# Procedure Name: directory_export
# Tactic: Discovery
# Technique: T1087.002
...
EXPORT_LOCAL_HOST_USERS=false
EXPORT_LOCAL_HOST_GROUPS=false

CMD_DSEXPORT="/usr/bin/dsexport"
...
PROCEDURE_NAME="directory_export"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --export-local-host-users)
            EXPORT_LOCAL_HOST_USERS=true
            ;;
        --export-local-host-groups)
            EXPORT_LOCAL_HOST_GROUPS=true
            ;;
            *)
...
SCRIPT:
  --export-local-host-users        Export the local host user information to a file
  --export-local-host-groups       Export the local host group information to a file
...
# Execute functions for --export-local-host-users
if [ "$EXPORT_LOCAL_HOST_USERS" = true ]; then
    core_debug_print "Executing functions for --export-local-host-users"
    result=$(execute_function execute_export_local_host_users)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --export-local-host-groups
if [ "$EXPORT_LOCAL_HOST_GROUPS" = true ]; then
    core_debug_print "Executing functions for --export-local-host-groups"
    result=$(execute_function execute_export_local_host_groups)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="directory_export"
...
# Function: execute_export_local_host_users
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_export_local_host_users() {
    local result
    result=$(dsexport local_users.txt /Local/Default dsRecTypeStandard:Users 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_export_local_host_groups
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_export_local_host_groups() {
    local result
    result=$(dsexport local_groups.txt /Local/Default dsRecTypeStandard:Groups 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
