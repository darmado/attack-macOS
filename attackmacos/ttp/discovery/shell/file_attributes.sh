...
# Tactic: Discovery
# Technique: T1082
# GUID: 19d4d21a-cb3f-4e57-bafd-cc6d019070be
# Intent: Get attributes of files and directories. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
ITERATE_THROUGH_A_DIRECTORY_TO_GETFILEINFO=false

CMD_GETFILEINFO="/usr/bin/GetFileInfo"
...
PROCEDURE_NAME="file_attributes"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --iterate-through-a-directory-to-getfileinfo)
            ITERATE_THROUGH_A_DIRECTORY_TO_GETFILEINFO=true
            ;;
            *)
...
SCRIPT:
  --iterate-through-a-directory-to-getfileinfo A bash or zsh oneliner can provide an attacker with information about specific files of interest.
...
# Execute functions for --iterate-through-a-directory-to-getfileinfo
if [ "$ITERATE_THROUGH_A_DIRECTORY_TO_GETFILEINFO" = true ]; then
    core_debug_print "Executing functions for --iterate-through-a-directory-to-getfileinfo"
    result=$(execute_function execute_iterate_through_directory_getfileinfo)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="file_attributes"
...
# Function: execute_iterate_through_directory_getfileinfo
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_iterate_through_directory_getfileinfo() {
    local result
    result=$(for FILE in ~/Downloads/*; do echo $(GetFileInfo $FILE) >> fileinfo.txt; sleep 2; done 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
