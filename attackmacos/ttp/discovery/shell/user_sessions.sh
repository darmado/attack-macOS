#!/bin/sh
# POSIX-compliant
# Procedure Name: user_sessions
# Tactic: Discovery
# Technique: T1087
# GUID: 73519d6a-2e27-46e4-bdbf-8ec6455a76ae
# Intent: Show last user logins and TTYs. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
TTP_ID="T1087" #replace with you coresponding ttp_id
...
ENUMERATE_THE_USERS_WHO_ARE_CURRENTLY_LOGGED_INTO_THE_SYSTEM=false
ENUMERATE_ALL_USER_ACCOUNTS_THAT_HAVE_LOGGED_INTO_THE_SYSTEM_PREVIOUSL=false
ENUMERATE_ALL_HOSTS_THAT_HAVE_REMOTELY_LOGGED_INTO_THE_SYSTEM_BEFORE=false
...
PROCEDURE_NAME="user_sessions"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --enumerate-the-users-who-are-currently-logged-into-the-system)
            ENUMERATE_THE_USERS_WHO_ARE_CURRENTLY_LOGGED_INTO_THE_SYSTEM=true
            ;;
        --enumerate-all-user-accounts-that-have-logged-into-the-system-previousl)
            ENUMERATE_ALL_USER_ACCOUNTS_THAT_HAVE_LOGGED_INTO_THE_SYSTEM_PREVIOUSL=true
            ;;
        --enumerate-all-hosts-that-have-remotely-logged-into-the-system-before)
            ENUMERATE_ALL_HOSTS_THAT_HAVE_REMOTELY_LOGGED_INTO_THE_SYSTEM_BEFORE=true
            ;;
            *)
...
SCRIPT:
  --enumerate-the-users-who-are-currently-logged-into-the-system The following command will display sessions that are currently active.
  --enumerate-all-user-accounts-that-have-logged-into-the-system-previousl The last command can be used to output users who have previously logged in, by specifying the tty in
  --enumerate-all-hosts-that-have-remotely-logged-into-the-system-before An attacker can use 'last' with a filter to retrieve the connection date and remote host information
...
# Execute functions for --enumerate-the-users-who-are-currently-logged-into-the-system
if [ "$ENUMERATE_THE_USERS_WHO_ARE_CURRENTLY_LOGGED_INTO_THE_SYSTEM" = true ]; then
    core_debug_print "Executing functions for --enumerate-the-users-who-are-currently-logged-into-the-system"
    result=$(execute_function execute_enumerate_users_who_are_currently_logged_into_system)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --enumerate-all-user-accounts-that-have-logged-into-the-system-previousl
if [ "$ENUMERATE_ALL_USER_ACCOUNTS_THAT_HAVE_LOGGED_INTO_THE_SYSTEM_PREVIOUSL" = true ]; then
    core_debug_print "Executing functions for --enumerate-all-user-accounts-that-have-logged-into-the-system-previousl"
    result=$(execute_function execute_enumerate_all_user_accounts_that_have_logged_into_system_previously)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --enumerate-all-hosts-that-have-remotely-logged-into-the-system-before
if [ "$ENUMERATE_ALL_HOSTS_THAT_HAVE_REMOTELY_LOGGED_INTO_THE_SYSTEM_BEFORE" = true ]; then
    core_debug_print "Executing functions for --enumerate-all-hosts-that-have-remotely-logged-into-the-system-before"
    result=$(execute_function execute_enumerate_all_hosts_that_have_remotely_logged_into_system_before)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="user_sessions"
...
# Function: execute_enumerate_users_who_are_currently_logged_into_system
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_enumerate_users_who_are_currently_logged_into_system() {
    local result
    result=$(last | grep "still logged in" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_enumerate_all_user_accounts_that_have_logged_into_system_previously
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_enumerate_all_user_accounts_that_have_logged_into_system_previously() {
    local result
    result=$(last -t console 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_enumerate_all_hosts_that_have_remotely_logged_into_system_before
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_enumerate_all_hosts_that_have_remotely_logged_into_system_before() {
    local result
    result=$(last | grep -E '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
