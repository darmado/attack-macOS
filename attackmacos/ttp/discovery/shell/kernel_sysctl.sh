#!/bin/sh
# POSIX-compliant
# Procedure Name: kernel_sysctl
# Tactic: Discovery
# Technique: T1082
# GUID: 5b298565-0832-42a2-a244-00f32635a38a
# Intent: Get macOS hardware model information. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
USE_SYSCTL_TO_GATHER_MACOS_HARDWARE_INFO=false
...
PROCEDURE_NAME="kernel_sysctl"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --use-sysctl-to-gather-macos-hardware-info)
            USE_SYSCTL_TO_GATHER_MACOS_HARDWARE_INFO=true
            ;;
            *)
...
SCRIPT:
  --use-sysctl-to-gather-macos-hardware-info sysctl can be used to gather interesting macOS host data, including hardware information, memory siz
...
# Execute functions for --use-sysctl-to-gather-macos-hardware-info
if [ "$USE_SYSCTL_TO_GATHER_MACOS_HARDWARE_INFO" = true ]; then
    core_debug_print "Executing functions for --use-sysctl-to-gather-macos-hardware-info"
    result=$(execute_function execute_use_sysctl_gather_macos_hardware_info)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="kernel_sysctl"
...
# Function: execute_use_sysctl_gather_macos_hardware_info
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_sysctl_gather_macos_hardware_info() {
    local result
    result=$(sysctl -n hw.model 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
