...
GENERATE_PAYLOAD_DIRECTORY_SHLAYER=false
GENERATE_DIRECTORY_BASED_ON_TEMPLATE_FILE_BUNDLORE=false
...
PROCEDURE_NAME="temp_path"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --generate-payload-directory-shlayer)
            GENERATE_PAYLOAD_DIRECTORY_SHLAYER=true
            ;;
        --generate-directory-based-on-template-file-bundlore)
            GENERATE_DIRECTORY_BASED_ON_TEMPLATE_FILE_BUNDLORE=true
            ;;
            *)
...
SCRIPT:
  --generate-payload-directory-shlayer The following command can be used to generate a random directory name for staging payloads
  --generate-directory-based-on-template-file-bundlore The following command can be used to generate a unique directory based on a template
...
# Execute functions for --generate-payload-directory-shlayer
if [ "$GENERATE_PAYLOAD_DIRECTORY_SHLAYER" = true ]; then
    core_debug_print "Executing functions for --generate-payload-directory-shlayer"
    result=$(execute_function execute_generate_payload_directory_shlayer)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --generate-directory-based-on-template-file-bundlore
if [ "$GENERATE_DIRECTORY_BASED_ON_TEMPLATE_FILE_BUNDLORE" = true ]; then
    core_debug_print "Executing functions for --generate-directory-based-on-template-file-bundlore"
    result=$(execute_function execute_generate_directory_based_template_file_bundlore)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_generate_payload_directory_shlayer
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_generate_payload_directory_shlayer() {
    local result
    result=$(export tmpDir="$(mktemp -d /tmp/XXXXXXXXXXXX)" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_generate_directory_based_template_file_bundlore
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_generate_directory_based_template_file_bundlore() {
    local result
    result=$(TMP_DIR="mktemp -d -t x" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
