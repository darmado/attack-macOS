#!/bin/sh
# POSIX-compliant
# Procedure Name: launch_services_registry
# Tactic: Discovery
# Technique: T1082
...
FORCE_AN_UPDATE_OF_THE_LAUNCH_SERVICES_DATABASE=false
GET_A_LIST_OF_APPS_AND_THEIR_BINDINGS=false
DELETE_THE_LAUNCH_SERVICES_DATABASE=false

CMD_LSREGISTER="/System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/LaunchServices.framework/Versions/A/Support/lsregister"
...
PROCEDURE_NAME="launch_services_registry"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --force-an-update-of-the-launch-services-database)
            FORCE_AN_UPDATE_OF_THE_LAUNCH_SERVICES_DATABASE=true
            ;;
        --get-a-list-of-apps-and-their-bindings)
            GET_A_LIST_OF_APPS_AND_THEIR_BINDINGS=true
            ;;
        --delete-the-launch-services-database)
            DELETE_THE_LAUNCH_SERVICES_DATABASE=true
            ;;
            *)
...
SCRIPT:
  --force-an-update-of-the-launch-services-database The -f flag can be used to force an update of the Launch Services database. This can be used to quic
  --get-a-list-of-apps-and-their-bindings The -dump flag can be used to get a list of apps and their bindings
  --delete-the-launch-services-database The -delete flag can be used to delete the Launch Services database to impact normal operation of th
...
# Execute functions for --force-an-update-of-the-launch-services-database
if [ "$FORCE_AN_UPDATE_OF_THE_LAUNCH_SERVICES_DATABASE" = true ]; then
    core_debug_print "Executing functions for --force-an-update-of-the-launch-services-database"
    result=$(execute_function execute_force_update_launch_services_database)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --get-a-list-of-apps-and-their-bindings
if [ "$GET_A_LIST_OF_APPS_AND_THEIR_BINDINGS" = true ]; then
    core_debug_print "Executing functions for --get-a-list-of-apps-and-their-bindings"
    result=$(execute_function execute_get_list_apps_their_bindings)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --delete-the-launch-services-database
if [ "$DELETE_THE_LAUNCH_SERVICES_DATABASE" = true ]; then
    core_debug_print "Executing functions for --delete-the-launch-services-database"
    result=$(execute_function execute_delete_launch_services_database)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="launch_services_registry"
...
# Function: execute_force_update_launch_services_database
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_force_update_launch_services_database() {
    local result
    result=$(/System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/LaunchServices.framework/Versions/A/Support/lsregister -f 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_get_list_apps_their_bindings
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_get_list_apps_their_bindings() {
    local result
    result=$(/System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/LaunchServices.framework/Versions/A/Support/lsregister -dump | grep -E "path:|bindings:|name: | more" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_delete_launch_services_database
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_delete_launch_services_database() {
    local result
    result=$(lsregister -delete 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
