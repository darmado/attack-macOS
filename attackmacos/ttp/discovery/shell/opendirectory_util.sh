#!/bin/sh
# POSIX-compliant
# Procedure Name: opendirectory_util
# Tactic: Discovery
# Technique: T1082
# GUID: 03c99b74-dbfd-48c2-888d-c3f97c81e6d8
# Intent: odutil allows caller to examine or change state of opendirectoryd Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
LISTING_THE_AVAILABLE_NODE_NAMES=false
RETRIEVES_ACTIVE_SESSION=false
RETRIEVES_DEFAULT_SEARCH_POLICY=false
RETRIEVES_CONTACT_SEARCH_POLICY=false

CMD_ODUTIL="/usr/bin/odutil"
...
PROCEDURE_NAME="opendirectory_util"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --listing-the-available-node-names)
            LISTING_THE_AVAILABLE_NODE_NAMES=true
            ;;
        --retrieves-active-session)
            RETRIEVES_ACTIVE_SESSION=true
            ;;
        --retrieves-default-search-policy)
            RETRIEVES_DEFAULT_SEARCH_POLICY=true
            ;;
        --retrieves-contact-search-policy)
            RETRIEVES_CONTACT_SEARCH_POLICY=true
            ;;
            *)
...
SCRIPT:
  --listing-the-available-node-names List all available node names
  --retrieves-active-session       Retrieves the all active sessions
  --retrieves-default-search-policy Retrieves the configuration of "Default search policy"
  --retrieves-contact-search-policy Retrieves the configuration of "Contact search policy"
...
# Execute functions for --listing-the-available-node-names
if [ "$LISTING_THE_AVAILABLE_NODE_NAMES" = true ]; then
    core_debug_print "Executing functions for --listing-the-available-node-names"
    result=$(execute_function execute_listing_available_node_names)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --retrieves-active-session
if [ "$RETRIEVES_ACTIVE_SESSION" = true ]; then
    core_debug_print "Executing functions for --retrieves-active-session"
    result=$(execute_function execute_retrieves_active_session)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --retrieves-default-search-policy
if [ "$RETRIEVES_DEFAULT_SEARCH_POLICY" = true ]; then
    core_debug_print "Executing functions for --retrieves-default-search-policy"
    result=$(execute_function execute_retrieves_default_search_policy)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --retrieves-contact-search-policy
if [ "$RETRIEVES_CONTACT_SEARCH_POLICY" = true ]; then
    core_debug_print "Executing functions for --retrieves-contact-search-policy"
    result=$(execute_function execute_retrieves_contact_search_policy)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="opendirectory_util"
...
# Function: execute_listing_available_node_names
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_listing_available_node_names() {
    local result
    result=$(odutil show nodenames 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_retrieves_active_session
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_retrieves_active_session() {
    local result
    result=$(odutil show sessions 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_retrieves_default_search_policy
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_retrieves_default_search_policy() {
    local result
    result=$(odutil show configuration /Search 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_retrieves_contact_search_policy
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_retrieves_contact_search_policy() {
    local result
    result=$(odutil show configuration /Contacts 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
