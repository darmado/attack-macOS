#!/bin/sh
# POSIX-compliant
# Procedure Name: kernel_extensions
# Tactic: Discovery
# Technique: T1082
# GUID: f91cfbe0-0d27-48f4-b9c9-08ba061d643a
# Intent: Display the status of loaded kernal extensions (kexts) Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
LIST_KERNEL_EXTENSIONS=false

CMD_KEXTSTAT="/usr/sbin/kextstat"
...
PROCEDURE_NAME="kernel_extensions"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --list-kernel-extensions         Uses kexstat showloaded to display kernel extensions and address in kernel memory it has been loaded
...
# Execute functions for --list-kernel-extensions
if [ "$LIST_KERNEL_EXTENSIONS" = true ]; then
    core_debug_print "Executing functions for --list-kernel-extensions"
    result=$(execute_function execute_list_kernel_extensions)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="kernel_extensions"
...
# Function: execute_list_kernel_extensions
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_list_kernel_extensions() {
    local result
    result=$(kexstat 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
