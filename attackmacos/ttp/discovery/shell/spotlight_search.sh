#!/bin/sh
# POSIX-compliant
# Procedure Name: spotlight_search
# Tactic: Discovery
# Technique: T1083
# GUID: 391be572-fbda-4ac7-9c13-e06cc0868454
# Intent: Locate files using the Spotlight database. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
TTP_ID="T1083" #replace with you coresponding ttp_id
...
USE_MDFIND_TO_PROVIDE_LIVE_UPDATES_TO_THE_NUMBER_OF_FILES_MATCHING_THE=false
USE_MDFIND_TO_SEARCH_FOR_AWS_KEYS=false
USE_MDFIND_TO_SEARCH_FOR_APPS_TO_INFECT=false
...
PROCEDURE_NAME="spotlight_search"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --use-mdfind-to-provide-live-updates-to-the-number-of-files-matching-the)
            USE_MDFIND_TO_PROVIDE_LIVE_UPDATES_TO_THE_NUMBER_OF_FILES_MATCHING_THE=true
            ;;
        --use-mdfind-to-search-for-aws-keys)
            USE_MDFIND_TO_SEARCH_FOR_AWS_KEYS=true
            ;;
        --use-mdfind-to-search-for-apps-to-infect)
            USE_MDFIND_TO_SEARCH_FOR_APPS_TO_INFECT=true
            ;;
            *)
...
SCRIPT:
  --use-mdfind-to-provide-live-updates-to-the-number-of-files-matching-the A bash or zsh oneliner can cause mdfind to provide an attacker with live updates to the number of fi
  --use-mdfind-to-search-for-aws-keys Allows an attacker to query the filesystem via the CommandLine/Terminal to search for AWS keys.
  --use-mdfind-to-search-for-apps-to-infect Allows an attacker to determine if specific applications are installed and can be leveraged
...
# Execute functions for --use-mdfind-to-provide-live-updates-to-the-number-of-files-matching-the
if [ "$USE_MDFIND_TO_PROVIDE_LIVE_UPDATES_TO_THE_NUMBER_OF_FILES_MATCHING_THE" = true ]; then
    core_debug_print "Executing functions for --use-mdfind-to-provide-live-updates-to-the-number-of-files-matching-the"
    result=$(execute_function execute_use_mdfind_provide_live_updates_number_files_matching_query)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --use-mdfind-to-search-for-aws-keys
if [ "$USE_MDFIND_TO_SEARCH_FOR_AWS_KEYS" = true ]; then
    core_debug_print "Executing functions for --use-mdfind-to-search-for-aws-keys"
    result=$(execute_function execute_use_mdfind_search_aws_keys)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --use-mdfind-to-search-for-apps-to-infect
if [ "$USE_MDFIND_TO_SEARCH_FOR_APPS_TO_INFECT" = true ]; then
    core_debug_print "Executing functions for --use-mdfind-to-search-for-apps-to-infect"
    result=$(execute_function execute_use_mdfind_search_apps_infect)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="spotlight_search"
...
# Function: execute_use_mdfind_provide_live_updates_number_files_matching_query
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_mdfind_provide_live_updates_number_files_matching_query() {
    local result
    result=$(mdfind -live passw 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_use_mdfind_search_aws_keys
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_mdfind_search_aws_keys() {
    local result
    result=$(mdfind 'kMDItemTextContext == AKIA || kMDItemDisplayName = *AKIA* -onlyin ~' 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_use_mdfind_search_apps_infect
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_use_mdfind_search_apps_infect() {
    local result
    result=$(set appId to do shell script "mdfind kMDItemCFBundleIdentifier = '" & bundleId & "'" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
