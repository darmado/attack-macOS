...
TTP_ID="T1005" #replace with you coresponding ttp_id
...
DISPLAY_LOGIN_ITEMS=false
RESET_LOGIN_ITEMS_TO_DEFAULTS=false
...
PROCEDURE_NAME="sfltool"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --display-login-items)
            DISPLAY_LOGIN_ITEMS=true
            ;;
        --reset-login-items-to-defaults)
            RESET_LOGIN_ITEMS_TO_DEFAULTS=true
            ;;
            *)
...
SCRIPT:
  --display-login-items            Identify all current login and background items configured on the system.
  --reset-login-items-to-defaults  Reset all third-party Login Items and revert to installation defaults.
...
# Execute functions for --display-login-items
if [ "$DISPLAY_LOGIN_ITEMS" = true ]; then
    core_debug_print "Executing functions for --display-login-items"
    result=$(execute_function execute_display_login_items)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --reset-login-items-to-defaults
if [ "$RESET_LOGIN_ITEMS_TO_DEFAULTS" = true ]; then
    core_debug_print "Executing functions for --reset-login-items-to-defaults"
    result=$(execute_function execute_reset_login_items_defaults)
    raw_output="${raw_output}${result}"
fi
...
# Function: execute_display_login_items
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_display_login_items() {
    local result
    result=$(sfltool dumpbtm 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}

# Function: execute_reset_login_items_defaults
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_reset_login_items_defaults() {
    local result
    result=$(sfltool resetbtm 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
