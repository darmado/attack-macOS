...
TACTIC="Command and Control" #replace with you coresponding tactic
TTP_ID="T1071.001" #replace with you coresponding ttp_id
...
INPUT_OUTPUT_PATH=""
INPUT_TARGET_DIR=""
HTTPS_IGNORE_CERT=false
SAVE_TO_DOWNLOADS_ALIAS=false
SAVE_VIA_DIR=false

CMD_NSCURL="/usr/bin/nscurl"
INPUT_URL=""
INPUT_OUTPUT_PATH=""
INPUT_TARGET_DIR=""
...
PROCEDURE_NAME="nscurl"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        ...
        --target-dir)
            if [ -n "$2" ] && [ "$2" != "${2#-}" ]; then
                ...
                ;;
        --https-ignore-cert)
            HTTPS_IGNORE_CERT=true
            ;;
        --save-to-downloads-alias)
            SAVE_TO_DOWNLOADS_ALIAS=true
            ;;
        --save-via-dir)
            SAVE_VIA_DIR=true
            ;;
...
SCRIPT:
  --url VALUE                      Target HTTPS URL for fetch modes (required with fetch flags)
  --output-path FILE_PATH          Local file path for --https-ignore-cert (-o target)
  --target-dir VALUE               Directory for --save-via-dir (-dir)
  --https-ignore-cert              Fetch URL ignoring TLS validation (-k) and write to --output-path
  --save-to-downloads-alias        Fetch using nscurl -dl (downloads folder alias on this host)
  --save-via-dir                   Fetch using nscurl -dir to --target-dir (see --url)
...
# Execute functions for --url
if [ "$URL" = true ]; then
    core_debug_print "Executing functions for --url"
fi

# Execute functions for --output-path
if [ "$OUTPUT_PATH" = true ]; then
    core_debug_print "Executing functions for --output-path"
fi

# Execute functions for --target-dir
if [ "$TARGET_DIR" = true ]; then
    core_debug_print "Executing functions for --target-dir"
fi

# Execute functions for --https-ignore-cert
if [ "$HTTPS_IGNORE_CERT" = true ]; then
    core_debug_print "Executing functions for --https-ignore-cert"
    result=$(execute_function nscurl_https_ignore_cert)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --save-to-downloads-alias
if [ "$SAVE_TO_DOWNLOADS_ALIAS" = true ]; then
    core_debug_print "Executing functions for --save-to-downloads-alias"
    result=$(execute_function nscurl_save_dl)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --save-via-dir
if [ "$SAVE_VIA_DIR" = true ]; then
    core_debug_print "Executing functions for --save-via-dir"
    result=$(execute_function nscurl_save_dir)
    raw_output="${raw_output}${result}"
fi
...
process_input_arguments() {
    ...
    if [ -n "${INPUT_OUTPUT_PATH}" ]; then
        # Process string input
        OUTPUT_PATH_ARG="${INPUT_OUTPUT_PATH}"
    fi
    
    # Process --target-dir argument
    if [ -n "${INPUT_TARGET_DIR}" ]; then
        # Process string input
        TARGET_DIR_ARG="${INPUT_TARGET_DIR}"
    fi
}


# Function: nscurl_https_ignore_cert
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
...
nscurl_https_ignore_cert() {
    if [ -z "$INPUT_URL" ] || [ -z "$INPUT_OUTPUT_PATH" ]; then
        $CMD_PRINTF "ERROR|nscurl|--https-ignore-cert requires --url and --output-path\n"
        return 1
    fi
    local result
    result=$("$CMD_NSCURL" -k "$INPUT_URL" -o "$INPUT_OUTPUT_PATH" 2>&1) || true
...
nscurl_save_dl() {
    if [ -z "$INPUT_URL" ]; then
        $CMD_PRINTF "ERROR|nscurl|--save-to-downloads-alias requires --url\n"
        return 1
    fi
    local result
    result=$("$CMD_NSCURL" "$INPUT_URL" -dl 2>&1) || true
...
nscurl_save_dir() {
    if [ -z "$INPUT_URL" ] || [ -z "$INPUT_TARGET_DIR" ]; then
        $CMD_PRINTF "ERROR|nscurl|--save-via-dir requires --url and --target-dir\n"
        return 1
    fi
    local result
    result=$("$CMD_NSCURL" "$INPUT_URL" -dir "$INPUT_TARGET_DIR" 2>&1) || true
    $CMD_PRINTF "NSCURL|mode|dir|url|%s|dir|%s\n" "$INPUT_URL" "$INPUT_TARGET_DIR"
    $CMD_PRINTF "NSCURL|stdout_stderr|%s\n" "$result"
    return 0
}
